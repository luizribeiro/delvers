...
#[derive(Parser, Debug)]
#[command(name = "neth4x0rs", about = "A cooperative multi-player roguelike")]
struct Cli {
...
fn default_socket_path() -> PathBuf {
    if let Ok(p) = std::env::var("NETH4X0RS_SOCKET") {
        return PathBuf::from(p);
    }
    let dir = std::env::var("XDG_RUNTIME_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from("/tmp"));
    dir.join("neth4x0rs.sock")
}
...
fn spawn_server(socket: &str) -> Result<()> {
    ...
    let log_path = std::env::var("NETH4X0RS_LOG").unwrap_or_else(|_| "/tmp/neth4x0rs.log".into());
